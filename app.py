...
with st.sidebar:
    ...
    st.write("1. Définir la cadence")
    st.write("2. Choisir groupes/comptes")
...
tabs = st.tabs(["1. Cadence", "2. Comptes", "3. Contenus", "4. Preview", "5. Envoi"])
...
with tabs[0]:
    ...
    ready_accounts = bool(st.session_state.get("grouped_accounts"))
    ready_posts = bool(st.session_state.get("selected_posts"))
    c1, c2, c3 = st.columns(3)
    c1.markdown(f"<div class='step-note'><b>Comptes</b><br>{'OK' if ready_accounts else 'À sélectionner'}</div>", unsafe_allow_html=True)
    c2.markdown(f"<div class='step-note'><b>Posts/photos</b><br>{'OK' if ready_posts else 'À sélectionner'}</div>", unsafe_allow_html=True)
    c3.markdown(f"<div class='step-note'><b>Envoi réel</b><br>{'Bloqué par dry-run' if dry_run else 'Confirmation requise'}</div>", unsafe_allow_html=True)
...
with tabs[1]:
    ...
    section_intro(
        ...
        "Un compte appartient à un seul groupe pour garder la preview lisible et éviter les doublons.",
    )
    if not client:
    ...
    if not accounts:
        ...
        st.caption("Groupes existants : " + ", ".join(group_options))
        selected_group_filters = st.multiselect(
            "Voir comptes dans groupes",
            options=group_options,
            default=st.session_state.get("selected_group_filters", group_options[:1]),
            key="selected_group_filters",
        )
        if selected_group_filters:
            with st.expander("Comptes dans les groupes sélectionnés", expanded=True):
                for group_name in selected_group_filters:
                    group_accounts = [
                        account for account in accounts
                        if (account.get("group_name") or "tous") == group_name
                    ]
                    names = [account_label(account) for account in group_accounts]
                    st.markdown(
                        f"**{group_name}** ({len(group_accounts)} comptes)  \n"
                        + (", ".join(names) if names else "Aucun compte dans ce groupe.")
                    )
        st.markdown("#### Comptes")
        st.caption("Coche les comptes à utiliser, puis choisis juste leur groupe dans la colonne Groupe.")
        rows = []
        selected_ids = {int(a["id"]) for a in st.session_state.get("selected_accounts", [])}
        default_select_all = not selected_ids and not selected_group_filters
        for account in accounts:
            ...
            rows.append(
                {
                    "use": active and (selected_by_group or default_select_all or int(account["id"]) in selected_ids),
                    "id": int(account["id"]),
        ...
        render_group_cards(db.list_groups(), grouped)
...
with tabs[2]:
    ...
    section_intro(
        "Étape 3",
        "Ajoute les textes, les variables, les dossiers média et les replies en chaîne.",
    ...
    if not posts:
        ...
        edited_posts = st.data_editor(
            ...
            column_config={
                ...
                "active": st.column_config.CheckboxColumn("Actif"),
                "id": st.column_config.NumberColumn("ID", disabled=True),
                "caption": st.column_config.TextColumn("Caption", disabled=True, width="large"),
                ...
                "photo_note": st.column_config.TextColumn("Photo note", width="medium"),
...
with tabs[3]:
    ...
    current = settings()
    enough_context = bool(st.session_state.get("selected_posts")) and bool(st.session_state.get("grouped_accounts"))
    if not enough_context:
        st.warning("Sélectionne comptes et posts avant preview.")
    ...
    if all_scheduled:
        ...
        with filter_col:
            ...
            view_mode = st.radio("Vue", ["Tout", "Par compte", "Par jour", "Failed"], horizontal=False)
...
with tabs[4]:
    ...
    section_intro(
        ...
        "L'envoi reste bloqué tant que dry-run est actif ou que les confirmations ne sont pas cochées.",
    )
    ...
    can_send = bool(preview) and ok_accounts and ok_posts and ok_times and confirm_text.strip().upper() == "DEMARRER"

    if dry_run:
        st.warning("Mode dry-run actif : bouton réel bloqué.")
